...
class LaserWanderer:
  ...
  def compute_cost(self, delta, rollout_pose, laser_msg):
    ...
    curr_rollout_angle_1 = curr_rollout_angle + 0.349066 # PP - adding 20 degrees to rollout angle
    curr_rollout_angle_2 = curr_rollout_angle - 0.349066 # PP - subtracting 20 degrees to rollout angle
    laser_index = (int)((curr_rollout_angle - laser_msg.angle_min) / laser_msg.angle_increment)
    laser_index_check = (int)((curr_rollout_angle_1 - laser_msg.angle_min) / laser_msg.angle_increment)
    laser_index_neg_check = (int)((curr_rollout_angle_2 - laser_msg.angle_min) / laser_msg.angle_increment)

    #laser_index_90 = (int)((0.785398 - laser_msg.angle_min) / laser_msg.angle_increment)
    #laser_index_neg_90 = (int)((-0.785398 - laser_msg.angle_min) / laser_msg.angle_increment)
    ...
    if laser_msg.ranges[laser_index_check] < np.abs(self.laser_offset):
      cost = cost + MAX_PENALTY
    
    if laser_msg.ranges[laser_index_neg_check] < np.abs(self.laser_offset):
      cost = cost + MAX_PENALTY
...
def main():
  ...
  laser_offset = 1.2 # Starting val: 1.0
  '''
